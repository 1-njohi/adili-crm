<template>
    <AuthenticatedLayout :title="`Edit Plot #${plot.plot_number}`">
...
const form = useForm({
    plot_number: props.plot.plot_number,
    size: props.plot.size || '',
    size_unit: props.plot.size_unit || 'acres',
    payment_tiers: props.plot.payment_tiers || [],
    custom_attributes: props.plot.custom_attributes || {},
    status: props.plot.status || 'available',
});
...
const submit = () => {
    form.put(`/owner/projects/${props.project.id}/plots/${props.plot.id}`);
};
</script>