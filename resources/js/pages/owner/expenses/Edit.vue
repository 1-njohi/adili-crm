    ...
    <AuthenticatedLayout :title="`Edit Expense`">
        <div class="bg-white rounded-lg shadow p-6 max-w-2xl">
            <form @submit.prevent="submit" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    <label class="block text-sm font-medium mb-1">Current Receipt</label>
                    <div v-if="form.existing_receipt" class="mb-2">
                        <a :href="`/storage/${form.existing_receipt}`" target="_blank" class="text-blue-600 hover:underline">
                            View Current Receipt
                        </a>
                    </div>
                    <label class="block text-sm font-medium mb-1">Replace Receipt (optional)</label>
...
const form = useForm({
    ...
    incurred_at: props.expense.incurred_at || new Date().toISOString().split('T')[0],
    type: props.expense.type || 'pre_launch',
    receipt: null,
    internal_notes: props.expense.internal_notes || '',
    existing_receipt: props.expense.receipt_path || null,
});

const submit = () => {
    form.put(`/owner/projects/${props.project.id}/expenses/${props.expense.id}`, {
        forceFormData: true,
    });
};
</script>